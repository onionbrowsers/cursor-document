...
> **最后更新：** 2026-03-26  
> **本次扫描：** 8 个 commands、10 条 rules（4 条 alwaysApply）、1 个 hook、11 个 skills、1 个 agent、3 个 scripts（含 README）  
...
| `commands/git-merge.md` | `/git-merge` | 拉取指定远程分支最新代码并合并到当前分支（--no-ff），支持 `--no-push`，工作区脏时拒绝执行，冲突时列出文件并终止 |
| `commands/gitlab-mr.md` | `/gitlab-mr` | 通过 GitLab MCP 自动生成 MR 标题与描述（Conventional Commits 规范），支持 `draft` 模式，创建前展示预览等待确认 |
...
| `skills/git-commit-merge-mr/` | 提交代码并生成MR、一键发版、commit merge mr | 三阶段全流程：①分析变更生成 commit message 并提交推送；②fetch 目标分支、pull、切回合并（--no-ff）；③调用 GitLab MCP 生成 MR 标题描述并创建。合并冲突时终止并给出解决指引 | GitLab MCP |
